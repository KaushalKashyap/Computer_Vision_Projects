...
cnts,_ = cv.findContours(thresholded.copy(), cv.RETR_TREE, cv.CHAIN_APPROX_NONE)
count1 = max(cnts, key=lambda x: cv.contourArea(x))
...
count_defects = 0
cv.drawContours(thresholded, cnts, -1, (0, 255, 0), 3)
